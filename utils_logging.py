...
@st.cache_resource(show_spinner="Connecting to the database",ttl=600)
def open_connection():
    return pymongo.MongoClient(os.environ['mongo_login_reg'], uuidRepresentation='standard')
    

def send_report(session_state, rated):

    try:
        mongoclient = open_connection()
        if rated:
